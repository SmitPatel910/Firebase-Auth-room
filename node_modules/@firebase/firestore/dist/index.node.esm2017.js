...
class IndexedDbPersistence {
     ...
     */
    allowTabSynchronization, persistenceKey, clientId, platform, lruParams, queue, serializer, sequenceNumberSyncer, 
    /**
    ...
    forceOwningTab) {
        ...
        if (platform.window && platform.window.localStorage) {
            ...
            if (forceOwningTab === false) {
                logError(LOG_TAG$2, 'LocalStorage is unavailable. As a result, persistence may not work ' +
    ...
    start() {
        ...
        return SimpleDb.openOrCreate(this.dbName, SCHEMA_VERSION, new SchemaConverter(this.serializer))
            ...
            return this.updateClientMetadataAndTryBecomePrimary(this.forceOwningTab);
        })
            .then(() => {
     ...
     * primary lease.
     */
    updateClientMetadataAndTryBecomePrimary(forceOwningTab = false) {
    ...
    detachWindowUnloadHook() {
        ...
        if (this.windowUnloadHandler) {
            debugAssert(typeof ((_a = this.window) === null || _a === void 0 ? void 0 : _a.removeEventListener) === 'function', "Expected 'window.removeEventListener' to be a function");
...
class PersistentWriteStream extends PersistentStream {
    ...
    writeHandshake() {
        ...
        debugAssert(this.lastStreamToken.isEqual(ByteString.EMPTY_BYTE_STRING), 'Stream token should be empty during handshake');
...
class SyncEngine {
    ...
    runTransaction(asyncQueue, updateFunction, deferred) {
        new TransactionRunner(asyncQueue, this.datastore, updateFunction, deferred).run();
    }
...
class MultiTabSyncEngine extends SyncEngine {
    constructor(localStore, remoteStore, datastore, sharedClientState, currentUser, maxConcurrentLimboResolutions) {
        super(localStore, remoteStore, datastore, sharedClientState, currentUser, maxConcurrentLimboResolutions);
        this.localStore = localStore;
    ...
    async synchronizeQueryViewsAndRaiseSnapshots(targets, transitionToPrimary) {
        ...
        for (const targetId of targets) {
            ...
            if (queries && queries.length !== 0) {
                ...
                debugAssert(transitionToPrimary, 'A secondary tab should never have an active view without an active target.');
...
class MemoryComponentProvider {
    ...
    createSyncEngine(cfg) {
        return new SyncEngine(this.localStore, this.remoteStore, cfg.datastore, this.sharedClientState, cfg.initialUser, cfg.maxConcurrentLimboResolutions);
    }
...
class IndexedDbComponentProvider extends MemoryComponentProvider {
    ...
    createSyncEngine(cfg) {
        const syncEngine = new MultiTabSyncEngine(this.localStore, this.remoteStore, cfg.datastore, this.sharedClientState, cfg.initialUser, cfg.maxConcurrentLimboResolutions);
    ...
    createPersistence(cfg) {
        ...
        return new IndexedDbPersistence(cfg.persistenceSettings.synchronizeTabs, persistenceKey, cfg.clientId, cfg.platform, LruParams.withCacheSize(cfg.persistenceSettings.cacheSizeBytes), cfg.asyncQueue, serializer, this.sharedClientState, cfg.persistenceSettings.forceOwningTab);
    }
    createSharedClientState(cfg) {
        if (cfg.persistenceSettings.durable &&
...
class DeleteFieldValueImpl extends SerializableFieldValue {
    ...
    _toFieldTransform(context) {
        if (context.dataSource === 2 /* MergeSet */) {
            ...
            debugAssert(context.path.length > 0, `${this._methodName}() at the top level should have already ` +
                'been handled.');
...
function parseScalarValue(value, context) {
    if (value === null) {
        ...
        return {
            referenceValue: context.serializer.toResourceName(value._key.path, value._databaseId)
        };
    }
    ...
    data(options) {
        ...
        if (!this._document) {
            ...
            if (this._converter) {
                ...
                const userDataWriter = new UserDataWriter(this._firestore._databaseId, this._firestore._areTimestampsInSnapshotsEnabled(), options.serverTimestamps || 'none', key => new DocumentReference(key, this._firestore));
...
class GrpcConnection {
    ...
    openStream(rpcName, token) {
        ...
        grpcStream.on('error', (grpcError) => {
            logWarn(LOG_TAG$e, 'GRPC stream error. Code:', grpcError.code, 'Message:', grpcError.message);
            const code = mapCodeFromRpcCode(grpcError.code);
